...
export const describe: Describe =
  'Search for properties and parameters in a blueprint'
...
export const handler: Handler<Options> = async ({
  ...
  const uniqueResults = searchResults.filter(
    (result, index, self) =>
      index ===
      self.findIndex((r) => r.path === result.path && r.name === result.name),
  )
  ...
  for (const result of uniqueResults) {
    ...
    logger.info(
      `${result.type.toUpperCase()}: ${result.name}${required}${methods} - ${displayPath}`,
    )
  }
...
function searchProperties(
  ...
  for (const prop of properties) {
    ...
    if (prop.itemProperties && Array.isArray(prop.itemProperties)) {
      results.push(
        ...searchProperties(prop.itemProperties, query, `${currentPath}[item]`),
      )
    }
  }
